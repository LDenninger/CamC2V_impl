...
#from inference.fifo_diffusion import prepare_latents_, prepare_batch, tensor2image, shift_latents
...
class CameraControlLVDM(DynamiCrafter):
    def __init__(self,
        ...
        if 'TemporalTransformer.attn2' in self.diffusion_model_trainable_param_list:
            for n, m in self.model.named_modules():
                if m.__class__.__name__ == 'BasicTransformerBlock' and getattr(m.attn2, "context_dim", None) is None:  # attn2 of TemporalTransformer BasicBlocks
                    for p in m.attn2.parameters():
                        p.requires_grad = True
        elif 'TemporalTransformer.attn1' in self.diffusion_model_trainable_param_list:
            for n, m in self.model.named_modules():
                if m.__class__.__name__ == 'BasicTransformerBlock' and getattr(m.attn2, "context_dim", None) is None is None:  # attn1 of TemporalTransformer BasicBlocks
                    print(n)
    ...
    def log_images(
        ...
        **kwargs,
        ...
        if use_fifo:
            assert "fifo_config" in kwargs, "Provide fifo_config for fifo diffusion to log_images()"
            batch_ = prepare_batch(batch, start_index=0, end_index=16)
            fifo_config = kwargs.get('fifo_config')
            video_name = str(Path(batch_["video_path"][0]).stem)
            latents_dir = str(Path(fifo_config['output_dir']) / video_name / "latents")
            ddim_steps = fifo_config["num_inference_steps"] * fifo_config["num_partitions"] 
        ...
        if sample:
            ...
            if use_fifo:
                assert "fifo_config" in kwargs, "Provide fifo_config for fifo diffusion to log_images()"
                fifo_config = kwargs.get("fifo_config")
                num_inference_steps = fifo_config["num_inference_steps"] * fifo_config["num_partitions"]
                num_partitions = fifo_config["num_partitions"]
                lookahead_denoising = fifo_config["lookahead_denoising"]
                output_dir = fifo_config["output_dir"]
                return self.sample_fifo(
                    batch = batch,
