...
def render_weight_cuda(
    ...
    """
    Returns per-pixel accumulated opacity (coverage) of all Gaussians:
        weight(x, y) = sum_i T_{i-1} * alpha_i  =  1 - T_final
    Implemented by a second rasterization pass with unit colors and black bg.
    """
    b, g, _ = gaussian_means.shape

    # Fake "SH coeffs" with n=1 so degree=0; values=1 so colors_precomp==1.
    fake_sh = torch.ones(
        (b, g, 3, 1),
    ...
    # Black background so the rendered RGB equals the accumulated transmittance*alpha sum.
    black_bg = torch.zeros((b, 3), dtype=gaussian_means.dtype, device=gaussian_means.device)
    ...
    rgb = render_cuda(
        ...
        gaussian_sh_coefficients=fake_sh,  # n=1, degree=0
        gaussian_opacities=gaussian_opacities,
        scale_invariant=scale_invariant,
        use_sh=False,  # forces colors_precomp path; all ones
    )
    #import ipdb; ipdb.set_trace()
    # All three channels are identical; take one and clamp for numerical stability.
    weight = rgb[:, 0:1].clamp(0.0, 1.0)
    return weight