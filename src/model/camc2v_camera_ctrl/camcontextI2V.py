...
from model.camc2v_camera_ctrl.base import CameraControlLVDM
from model.camc2v_camera_ctrl.multicondi2v_modified_modules import (
...
from model.camc2v_camera_ctrl.epipolar import Epipolar, EpipolarCrossAttention, pix2coord, VisualCrossAttention
...
class CamContext2Video(CameraControlLVDM):
    ...
    def __init__(self,
                    ...
                    visual_cross_attn_trainable: bool = False,
                    latent_adapter_trainable: bool = False,
                    ...
                    multi_latent_supervision_loss_scale: float = 0.0,
        ...
        self.latent_adapter_trainable = latent_adapter_trainable
        self.diffusion_model_trainable = diffusion_model_trainable
        ...
        if self.use_zero_convolution: # the zero conv should kernel size should be 1
        ...
        if self.diffusion_model_trainable:
            for name, param in self.model.diffusion_model.named_parameters():
                param.requires_grad_(True)
        elif "diffusion_model_trainable_param_list" in kwargs:
        ...
        for _name, _module in self.model.diffusion_model.named_modules():
            if _module.__class__.__name__ == 'TemporalTransformer':
                ...
                if _module.context_dim is None and _module.attn1.to_k.in_features != self.model.diffusion_model.init_attn[0].proj_in.out_channels:  # BasicTransformerBlock of TemporalTransformer, only self attn, context_dim=None
                    ...
                    if self.visual_cross_attn_config is not None:
                        visual_cross_attn = VisualCrossAttention(
                            query_dim = _module.attn1.to_k.in_features,
                            context_dim = 4,
                            **self.visual_cross_attn_config
                        )
                        _module.add_module('visual_cross_attn', visual_cross_attn)
                        for _, param in visual_cross_attn.named_parameters():
                            param.requires_grad_(visual_cross_attn_trainable)
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False,
        ...
        ## Add additional condition for cross attention ##
        if self.visual_cross_attn_config is not None or self.latent_adapter is not None:
            x = torch.cat([x, cond_frames], dim=2)

        ## Encode to latent space ##
        z = self.encode_first_stage(x)

        #x_ = self.decode_first_stage(z)
        #video_gt_reconst = Video.fromArray(x_[0,:,:T].permute(1,2,3,0).cpu().numpy(), name="GT Reconst")
        #video_rend_reconst = Video.fromArray(x_[0,:,T:].permute(1,2,3,0).cpu().numpy(), name="Rend Reconst")
        #video_save = video_gt_reconst | video_rend_reconst
        #video_save.save("debug.mp4")
        #import ipdb; ipdb.set_trace()
        ## Cut out additional condition from latents ""
        additional_condition = None
        if self.visual_cross_attn_config is not None:
            #import ipdb; ipdb.set_trace()
            z_cross_attn_cond = z[:, :, -(cond_frames.shape[1]+1):]
            z_mask = None
            additional_condition = {
                "condition_embedding": z_cross_attn_cond,
                "condition_mask": z_mask
            }
        if self.latent_adapter is not None:
            #import ipdb; ipdb.set_trace()
            z_context = z[:, :, -(cond_frames.shape[1]+1):]
            z_main = z[:,:,:-(cond_frames.shape[1]+1)]
            z_inp = z[:,:,-T:]
            z_adapt = self.latent_adapter(
                x = z_inp, context = z_context
            )
            z = torch.cat([z_main[:, :, :-T], z_adapt], dim=2)
            #z[:,:,-T:] = z_adapt
        ...
        ## Apply zero convolution to the latent condition ##
        if self.zero_convolution is not None:
            #import ipdb; ipdb.set_trace()
            original_cond = z[torch.arange(batch_size, device=device), :, cond_frame_index, :, :]
            original_cond = repeat(original_cond, "B C H W -> B C T H W", T=img_cat_cond.shape[2])
            img_cat_cond = self.zero_convolution(img_cat_cond)
            img_cat_cond = original_cond + img_cat_cond
        ...
        if return_cond_frame:
            if cond_frames is None:
                ...
                #cond_frames = ((rearrange(cond_frames, "B T H W C -> B C T H W")/255.)*2.) - 1. # B T H W C
    ...
    def get_cache3d_rendering(self, batch):
        #if type(self.cache3d) == NvbloxCache3D:
        #    _cond_frame_avail = 'cond_frames' in batch and batch['cond_frames'] is not None
        #    x = super().get_input(batch, self.first_stage_key)
        #    cam_intrinsics = super().get_input(batch, 'camera_intrinsics_depth').float()  # b, t, 3, 3
        #    w2c_RT_4x4 = super().get_input(batch, 'RT_depth').float()  # b, t, 4, 4
        #    
        #    if _cond_frame_avail:
        #        cond_frames = super().get_input(batch, 'cond_frames')
        #        cond_frames = (((cond_frames + 1.)/2.)*255).to(dtype=torch.uint8)
        #        cond_frames = rearrange(cond_frames, "B T C H W -> B T H W C")
        #        ref_img = ((rearrange(x[:,:,0:1], 'B C T H W -> B T H W C') + 1.)/2.*255).to(dtype=torch.uint8)
        #        cond_frames = torch.concatenate([ref_img, cond_frames], dim=1)
        #        cond_extrinsics = super().get_input(batch, 'RT_depth_cond')
        #        cond_extrinsics = torch.concatenate([w2c_RT_4x4[:,0:1], cond_extrinsics], dim=1)
        #        cond_extrinsics = torch.linalg.inv(cond_extrinsics)
        #        cond_intrinsics = super().get_input(batch, 'camera_intrinsics_depth_cond')
        #        cond_intrinsics = torch.concatenate([cam_intrinsics[:,0:1], cond_intrinsics], dim=1)
        #        cond_depth_maps = super().get_input(batch, 'depth_maps_cond')
        #        if cond_depth_maps.ndim == 5:
        #            cond_depth_maps = cond_depth_maps[:,:,0]
        #        depth_maps = super().get_input(batch, 'depth_maps')
        #        if depth_maps.ndim == 5:
        #            depth_maps = depth_maps[:,:,0]
        #        cond_depth_maps = torch.concatenate([depth_maps[:,0:1], cond_depth_maps], dim=1)
        #    else:
        #        cond_extrinsics = torch.linalg.inv(w2c_RT_4x4[:,0:1])
        #        cond_intrinsics = cam_intrinsics[:,0:1]
        #        cond_depth_maps = depth_maps[:,0:1]
        #        cond_frames = ((rearrange(x[:,:,0:1], 'B C T H W -> B T H W C') + 1.)/2.*255).to(dtype=torch.uint8)
        #    self.cache3d.update(
        #        images = cond_frames, # [B, F, C, H, W]
        #        depths = cond_depth_maps,   # [B, F,  H, W]
        #        extrinsics = cond_extrinsics,  # [B, F, 4, 4]
        #        intrinsics = cond_intrinsics,  # [B, F, 3, 3]
        #        initialize = False,
        #        update_color_mesh = False,
        #    )
        #    target_poses = super().get_input(batch, 'RT_depth')
        #    target_intrinsics = super().get_input(batch, 'camera_intrinsics_depth')
        #    target_poses = torch.linalg.inv(target_poses)
        #    rendered_cond_frames = self.cache3d.render(target_poses, target_intrinsics)
        #    rendered_cond_frames = torch.concatenate([ref_img, rendered_cond_frames[:,1:]], dim=1)
        #    rendered_cond_frames = (rendered_cond_frames/255.)*2. - 1.
        #    rendered_cond_frames = rearrange(rendered_cond_frames, 'B T H W C -> B T C H W')
        #    #import ipdb; ipdb.set_trace()
        #    #self.cache3d.print_timings()
        #    self.cache3d.reset()
    ...
    def configure_optimizers(self):
        ...
        if self.zero_convolution is not None:
            params.extend(list(self.zero_convolution.parameters()))
    ...
    def get_loss(self, pred, target, mean=True):
        ...
        if self.multi_latent_supervision_loss_scale > 0:
            if self.multi_latent_l2_loss is not None:
                loss += self.multi_latent_supervision_loss_scale * self.multi_latent_l2_loss
